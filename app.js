...
const fallbackQuestions = [
  ...
  },
  {
    ...
    "category": "Runtime",
    "level": "hard",
    "question": "objc_msgSend 为什么既动态又高效？",
    "answer": "objc_msgSend 的动态性来自运行时按 receiver 和 selector 查找 IMP，支持继承、分类、动态解析和消息转发。高效性主要来自方法缓存，常见调用命中 cache 后不需要遍历方法列表和父类链。它的性能边界在于缓存未命中、频繁动态替换实现、消息转发和过度依赖运行时黑魔法都会让调用路径变复杂。"
  },
  {
    "id": "ios-runtime-006",
    "category": "Runtime",
    "level": "hard",
    "question": "KVO 的实现原理和常见坑是什么？",
    "answer": "KVO 通常会在运行时创建被观察类的动态子类，重写 setter，在 setter 前后发送变更通知，并把对象的 isa 指向这个子类。常见坑是未按 KVO 兼容方式修改属性、重复移除观察、回调线程不符合 UI 预期、观察者生命周期管理不清，以及直接观察复杂派生状态导致通知语义混乱。"
  },
  {
    "id": "ios-runloop-005",
    "category": "RunLoop",
    "level": "hard",
    "question": "如何用 RunLoop 思路做卡顿监控？",
    "answer": "常见做法是在主线程 RunLoop 上注册 Observer，关注进入处理事件、准备休眠等状态，并用后台线程或信号量检测主线程是否长时间停留在某个活跃阶段。如果超过阈值，可以抓取主线程调用栈作为证据。难点是阈值选择、误报过滤、采样开销和线上隐私合规，不能只凭一次超时就断言卡顿根因。"
  },
  {
    "id": "ios-memory-006",
    "category": "内存",
    "level": "hard",
    "question": "weak 自动置 nil 大致是怎么做到的？",
    "answer": "weak 引用不会增加对象引用计数，运行时会在对象和 weak 指针之间维护辅助记录。对象 dealloc 时，运行时能找到指向它的 weak 引用位置，并把这些指针清零，避免悬垂指针。它解决的是对象释放后的野指针问题，不解决业务上的生命周期设计，也不能替代明确的所有权关系。"
  },
  {
    "id": "ios-memory-007",
    "category": "内存",
    "level": "hard",
    "question": "Timer 或 CADisplayLink 造成循环引用时怎么彻底处理？",
    "answer": "Timer 和 CADisplayLink 会强持有 target，target 再强持有 timer 时就会形成环。只把 block 里的 self 改 weak 不一定够，因为 timer 本身仍可能持有中间对象。更稳妥的做法是使用 weak proxy、block API 时确认捕获关系，并在明确生命周期节点 invalidate，同时把属性置空。"
  },
  {
    "id": "ios-swift-006",
    "category": "Swift",
    "level": "hard",
    "question": "Swift 中 any 和 some 的区别是什么？",
    "answer": "some Protocol 表示不透明类型，调用方不知道具体类型，但编译器知道且同一个返回位置必须保持具体类型一致。any Protocol 表示存在类型，可以装入不同的遵守者，但会有动态派发和类型擦除语义。返回值需要隐藏具体实现但保持静态类型能力时常用 some，需要集合里放多种实现或运行时替换时用 any。"
  },
  {
    "id": "ios-swift-007",
    "category": "Swift",
    "level": "hard",
    "question": "actor reentrancy 是什么，为什么会出问题？",
    "answer": "actor 保证同一时间只有一个执行片段访问其隔离状态，但遇到 await 时当前任务会挂起，actor 可以运行其他等待任务。恢复后，之前依赖的状态可能已经被其他任务改变，这就是 reentrancy 风险。处理方式是缩小 await 前后的状态假设，把关键检查放在 await 之后重新验证，或把不可中断的状态变更设计成同步片段。"
  },
  {
    "id": "ios-swift-008",
    "category": "Swift",
    "level": "hard",
    "question": "Sendable 解决什么问题？",
    "answer": "Sendable 用来表达一个值可以安全地跨并发边界传递。值类型、不可变数据或内部已同步的类型更容易满足 Sendable；带共享可变状态的引用类型需要格外谨慎。它不是自动加锁，而是让编译器帮助发现把不安全对象传进 Task、actor 或跨线程上下文的风险。"
  },
  {
    "id": "ios-uikit-008",
    "category": "UIKit",
    "level": "hard",
    "question": "iOS 事件响应链是怎么工作的？",
    "answer": "触摸事件先通过 hit-test 找到最合适的 view，事件会从这个 view 开始沿 responder chain 传递。UIView、UIViewController、UIWindow、UIApplication 都可以参与响应链。排查点击无效时要看 userInteractionEnabled、hidden、alpha、pointInside、hitTest、手势优先级、父视图裁剪和覆盖层，而不是只看按钮有没有绑定事件。"
  },
  {
    "id": "ios-uikit-009",
    "category": "UIKit",
    "level": "hard",
    "question": "什么是离屏渲染，为什么会影响性能？",
    "answer": "离屏渲染是 GPU 需要先把某些图层效果绘制到临时缓冲区，再合成到屏幕。圆角加裁剪、阴影、mask、复杂透明混合等都可能触发额外开销。优化时要区分真实瓶颈，常见手段是避免 masksToBounds 和 shadow 组合、使用 shadowPath、预渲染复杂内容、减少透明层级和过度绘制。"
  },
  {
    "id": "ios-uikit-010",
    "category": "UIKit",
    "level": "hard",
    "question": "复杂列表滑动卡顿怎么系统优化？",
    "answer": "先用 Instruments 或 FPS 数据确认瓶颈，再拆分数据计算、Auto Layout、文本排版、图片解码、圆角阴影、异步回调和主线程 IO。常见优化是预计算高度、降低约束复杂度、图片后台解码和按尺寸缓存、复用时取消旧请求、减少透明混合和离屏渲染。不要靠盲目异步化掩盖 cell 状态错乱。"
  },
  {
    "id": "ios-concurrency-008",
    "category": "并发",
    "level": "hard",
    "question": "异步任务取消应该怎么设计？",
    "answer": "取消应该是协作式的：调用方发出取消信号，任务在合适检查点停止后续工作、释放资源并返回一致状态。Swift Task、Operation、URLSessionTask 都有取消语义，但取消不等于立刻终止线程。业务层要处理取消后的回调、缓存写入、UI 状态回滚和重复请求，避免已经失效的结果覆盖新状态。"
  },
  {
    "id": "ios-concurrency-009",
    "category": "并发",
    "level": "hard",
    "question": "多线程共享状态时怎么选择锁、串行队列、barrier 或 actor？",
    "answer": "锁适合保护很小的临界区，但要控制锁顺序和避免持锁做耗时操作。串行队列适合把一组状态访问统一排队，语义清晰但可能成为瓶颈。barrier 适合读多写少的自建并发队列。actor 适合 Swift 并发体系下表达隔离状态，但要注意 await 后重入。选择标准是状态复杂度、调用频率、可取消性和与现有并发模型的一致性。"
  },
  {
    "id": "ios-network-006",
    "category": "网络",
    "level": "hard",
    "question": "Token 刷新遇到并发请求怎么处理？",
    "answer": "难点是多个请求同时发现 token 过期时不能一起刷新，也不能让旧 token 的失败结果覆盖新状态。常见做法是把刷新流程收敛成单飞任务，其他请求等待同一个刷新结果；刷新成功后重放可重试请求，失败则统一退出登录或返回鉴权错误。还要避免无限重试，并区分 401、网络失败和服务端业务错误。"
  },
  {
    "id": "ios-network-007",
    "category": "网络",
    "level": "hard",
    "question": "HTTP 缓存里的 ETag 和 Last-Modified 怎么用？",
    "answer": "ETag 是服务端给资源版本的标识，Last-Modified 是资源最后修改时间。客户端再次请求时可带 If-None-Match 或 If-Modified-Since，服务端资源未变化就返回 304，客户端继续使用本地缓存。移动端要结合 URLCache、业务缓存和接口语义，避免把用户态、鉴权态或强实时数据错误缓存。"
  },
  {
    "id": "ios-architecture-003",
    "category": "架构",
    "level": "hard",
    "question": "iOS 模块化拆分的边界怎么定？",
    "answer": "模块化不是按页面数量机械拆工程，而是按业务稳定边界、复用边界、依赖方向和构建成本来拆。底层能力模块应避免依赖业务，上层业务模块通过协议或门面访问公共能力。拆分后要控制循环依赖、资源归属、路由入口、二进制体积和 CI 构建成本，否则模块越多协作越重。"
  },
  {
    "id": "ios-architecture-004",
    "category": "架构",
    "level": "hard",
    "question": "如何保证复杂页面的状态一致性？",
    "answer": "关键是把页面状态建模成有限且可推导的结构，明确输入事件、异步副作用和状态输出。网络回调、用户操作、缓存恢复和生命周期事件都应通过同一套状态变更入口合并，避免多个地方直接改 UI。还要处理请求乱序、取消、重试、空态、错误态和局部刷新，否则复杂页面很容易出现旧数据覆盖新状态。"
  },
  {
    "id": "ios-storage-003",
    "category": "存储",
    "level": "hard",
    "question": "Core Data 多线程使用要注意什么？",
    "answer": "NSManagedObjectContext 有队列亲和性，不能随意跨线程访问。后台写入应使用 private queue context，通过 perform 或 performAndWait 进入对应队列。NSManagedObject 不适合直接跨 context 传递，通常传 objectID 再在目标 context 里重新获取。合并保存结果时要处理冲突、刷新 UI 的主线程边界和对象失效。"
  },
  {
    "id": "ios-storage-004",
    "category": "存储",
    "level": "hard",
    "question": "本地数据库迁移怎么降低风险？",
    "answer": "先区分轻量迁移和需要自定义映射的重迁移，明确旧版本覆盖范围和回滚策略。上线前要用真实旧库样本验证升级路径，避免只测新安装。迁移过程中要保护用户数据，失败时给出可恢复路径，必要时先备份再迁移。涉及大表变更时还要考虑耗时、磁盘空间、前后台中断和兼容多版本跳升。"
  },
  {
    "id": "ios-app-003",
    "category": "生命周期",
    "level": "hard",
    "question": "冷启动优化应该从哪里下手？",
    "answer": "先用 MetricKit、Instruments 或启动埋点拆出 pre-main、didFinishLaunching、首屏渲染等阶段。pre-main 关注动态库数量、类和分类加载、静态初始化；启动后关注同步 IO、数据库初始化、网络等待、图片解码和复杂布局。优化原则是延迟非首屏工作、合并必要初始化、减少主线程阻塞，并用数据验证每次改动。"
  },
  {
    "id": "ios-security-002",
    "category": "安全",
    "level": "hard",
    "question": "客户端本地敏感信息怎么保护？",
    "answer": "敏感 token、密钥和账号凭证应优先放 Keychain，并结合访问控制、过期策略和服务端吊销能力。不要把密钥硬编码在代码里当作绝对安全，客户端总有被逆向风险。日志、剪贴板、截图、崩溃上报和调试开关也可能泄漏敏感信息，安全设计要同时覆盖存储、传输、展示和诊断链路。"
  },
  {
    "id": "ios-debug-002",
    "category": "调试",
    "level": "hard",
    "question": "线上 OOM 怎么排查？",
    "answer": "OOM 通常没有普通 crash stack，要结合系统日志、MetricKit、内存曲线、场景埋点和用户操作路径还原。重点排查图片峰值、视频解码、大数组缓存、WKWebView、Core Data 批量对象、循环引用和 autorelease 峰值。修复时既要降低常驻内存，也要控制瞬时峰值，并为大资源场景做降级。"
  },
  {
    "id": "ios-debug-003",
    "category": "调试",
    "level": "hard",
    "question": "线上 crash 排查的证据链应该包括什么？",
    "answer": "先确认符号化后的崩溃栈、异常类型、线程、机型系统版本、App 版本和发生频率。再结合最近改动、用户路径、日志埋点、远端配置和复现条件缩小范围。不能只看崩溃栈顶一行就下结论，很多崩溃根因在异步回调、对象生命周期、数据格式或线程竞争上，需要把触发条件和代码路径串起来。"
  },
  {
    "id": "ios-performance-001",
    "category": "性能",
    "level": "hard",
    "question": "图片加载链路怎么优化内存和流畅度？",
    "answer": "核心是按展示尺寸请求或解码图片，避免原图直接进内存。网络下载、磁盘缓存、内存缓存、解码、缩放和 UI 设置要分层处理，解码和缩放尽量放后台，回主线程只做轻量赋值。列表中还要绑定模型标识、取消旧请求、控制预取和缓存成本，避免滑动时旧回调写错 cell 或瞬时内存暴涨。"
  },
  {
    "id": "ios-performance-002",
    "category": "性能",
    "level": "hard",
    "question": "App 包体积变大怎么分析和治理？",
    "answer": "先从构建产物拆分可执行文件、动态库、资源、图片、字体、模型和无用架构。治理手段包括删除未使用资源、压缩图片、按需下载大资源、控制第三方库、开启编译优化和检查符号/调试信息配置。包体积优化要结合下载包、安装包和运行时解压后的体积分别看，不能只看一个 ipa 文件大小。"
  }
];
