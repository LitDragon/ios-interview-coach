...
>
> 答：先看有没有所有权闭环。局部同步 Block 通常不需要 weak；GCD 延迟 Block 虽然执行后会释放，仍会在等待期间强持有 `self`。如果只是允许对象活到任务完成，可以强持有；如果页面退出后任务应失效，就应使用 weak、取消句柄或额外的生命周期门禁。

> **追问：** `__block` 和 `__weak` 有什么区别？
>
> 答：`__block` 解决的是“变量可在 Block 内修改”，变量会被包装进 byref 结构；在 ARC 下，`__block` 对象变量默认仍是强语义，除非同时声明为 weak。`__weak` 只表达弱所有权，不解决可修改性。
